...
| `5d07bf0e8e` | **perf: merge building_advisor+send_player_cities loops in begin_phase()** — `begin_phase()` ran two consecutive `phase_players_iterate` passes: (1) `building_advisor()` for human players (sets per-player advisor data); (2) `send_player_cities()` for all phase players (sends city packets). Neither reads the other player's advisor state; merged into one pass, eliminating one full O(N_phase) traversal per `begin_phase()` call (once per turn). |
| `e448e5b739` | **perf: merge gov/multiplier + research-auto-pick loops in end_phase()** — `end_phase()` ran two consecutive `phase_players_iterate` passes: (1) `government_change()` + multiplier reset/update; (2) research auto-pick when `researching == A_UNSET`. Neither reads the other player's output. Team pooled-research safety: `choose_tech()` sets `presearch->researching`; the next team-member's iteration finds it ≠ `A_UNSET` and skips — identical to two separate passes. Eliminates one full O(N_phase) traversal per `end_phase()` call (once per turn). |
| `ad7c46989f` | **perf: merge city_tc/unit_tc/finalize_unit_phase_beginning into one pass in begin_phase()** — `begin_phase()` ran two consecutive `phase_players_iterate` passes inside `if (is_new_phase)`: (1) `city_tc_effect_refresh()` (city tile-worker/specialist adjustments) + `unit_tc_effect_refresh()` (unit vision radius refresh); (2) `finalize_unit_phase_beginning()` (snapshots unit activity fields). None of these reads another player's output from the same pass. Merging eliminates one full O(N_phase) traversal per new phase. |
| `633bbd6ee9` | **perf: replace whole_map_iterate continent sizes with O(1) lookup; merge free_bulbs loop** — `adv_data_phase_init()` scanned every map tile to count continent/ocean sizes (O(N_tiles)), once per AI player per phase = O(P_ai × N_tiles) total. `wld.map.continent_sizes[]/ocean_sizes[]` already hold these counts (populated by `assign_continent_numbers()`). Replaced with a direct copy loop over continents/oceans: O(num_continents + num_oceans). Also merged the `free_bulbs = 0` reset loop into the preceding `auto_workers_player`/`last_activities` loop in `end_phase()`, eliminating one more O(N_phase) traversal. |
| `2c960d7c2f` | **fix: add missing `adv` declaration in `dai_build_adv_adjust`** — `f419f1c5` refactored `dai_build_adv_adjust()` to use `adv->stats.nplayers` but forgot to declare `struct adv_data *adv = adv_data_get(pplayer, nullptr)` in that function, causing a compilation error and all Railway deployments to fail from that commit onwards. |
| `71b0ca5da6` | **fix: use `int` loop variables + null guards for continent size copy in `adv_data_phase_init()`** — Changed `Continent_id c/o` loop variables to plain `int`, and wrapped both copy loops in null checks (`if (wld.map.continent_sizes != nullptr)`) to guard against edge cases before map generation completes. |
| `539fdc6d0c` | **fix: wrap `TEXAI_MSG_FIRST_ACTIVITIES` case body in braces** — GCC refuses to compile a VLA (`struct player *tex_dangerous[player_slot_count()]`) declared inside a `switch` case without braces when later case labels exist, because the declaration would be jumped over (error: "jump into scope of identifier with variably modified type"). Wrapping the entire case body in `{ }` puts the VLA in its own scope, fixing all Railway build failures since `ca30630ff1`. |
| `0a03bb8795` | **perf: merge 3 unit_list_iterate into one in dai_data_phase_begin()** — `daidata.c::dai_data_phase_begin()` ran three independent passes over `pplayer->units`: (1) workers-by-continent count; (2) diplomat-reservation BV marking; (3) ferry stats via `aiferry_init_stats()`. All three write distinct fields and none reads another's output; merged into a single traversal. Inlines `aiferry_init_stats()` in this path (function remains for other potential callers). Saves 2 full `unit_list_iterate` passes per AI player per turn. |
